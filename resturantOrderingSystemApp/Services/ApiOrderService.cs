...
namespace resturantOrderingSystemApp.Services
{
    public class ApiOrderService : IOrderService
    {
        ...
        public async Task<IEnumerable<Order>> GetOrders() //view all orders
        {
            try
            {
                ...
                var response = await _httpClient.GetAsync("v1/orders");

                if (response.IsSuccessStatusCode)
                {
                    var responseAsString = await response?.Content?.ReadAsStringAsync();
                    Console.WriteLine(responseAsString);
                    var tr = JsonSerializer.Deserialize<Data>(responseAsString);
        ...
        public async Task<OrderDetails> GetOrderDetails(long orderId) //view details for a specific Order
        {
            try
            {
                //var AuthKey = await SecureStorage.GetAsync("AuthKey");
                //string[] AuthenticationElements = AuthKey.Split(' ');
                _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic", "RmFpemFKYXZlZDpwNDU1dzByZA ==");
                var response = await _httpClient.GetAsync("v2/orderDetails/" + orderId);

                if (response.IsSuccessStatusCode)
                {
                    var responseAsString = await response?.Content?.ReadAsStringAsync();
                    Console.WriteLine(responseAsString);
                    var tr = JsonSerializer.Deserialize<DetailsData>(responseAsString);
                    Console.WriteLine(tr.data);
                    return tr.data;
        ...
        public async Task<bool> UpdateOrder(Order orderToUpdate) //update status for order
        {
           HttpResponseMessage response = new HttpResponseMessage { StatusCode = System.Net.HttpStatusCode.BadRequest };
            ...
            try
            {
                ...
                response = await _httpClient.PutAsync($"v3/orders/{orderToUpdate.orderId}",
                    new StringContent(JsonSerializer.Serialize(orderToUpdate), Encoding.UTF8, "application/vnd.api+json"));
                if (response.IsSuccessStatusCode)
                { 
                    return true;
                }
